...
RUNTIME_BUNDLE_SRC="$PKG_INSTALL_DIR/node_modules/@blackrock-ai/agent-runtime/dist/index.js"
RUNTIME_BUNDLE_DST="$FUNCTION_DIR/agent-runtime.js"
# Deno Deploy bundles the function at deploy time and CANNOT auth to GitHub
# Packages, so `npm:@blackrock-ai/...` imports fail. We side-step this by
# copying the runtime's prebuilt ESM bundle into the function directory and
# importing it via a relative path that Deno can resolve at bundle time.
...
// node_modules/@blackrock-ai/agent-runtime/dist/index.js at install time)
// because Deno Deploy cannot auth to GitHub Packages.
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "[dry-run] would copy runtime bundle: $RUNTIME_BUNDLE_SRC -> $RUNTIME_BUNDLE_DST"
  ...
  if [[ -f "$RUNTIME_BUNDLE_SRC" ]]; then
    ...
    echo "Copied runtime bundle: $RUNTIME_BUNDLE_DST"
  else
    echo "WARNING: runtime bundle not found at $RUNTIME_BUNDLE_SRC"
